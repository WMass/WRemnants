...
usage() {
    cat <<'EOF'
Run the alphaS W histmaker with the analysis defaults.

Usage:
  recipes/alpha_s/histmaker_w.sh [options]
...
if [[ -n "${postfix}" ]]; then
    cmd+=(--postfix "${postfix}")
    actual_file="${outdir}/mw_with_mu_eta_pt_${postfix}.hdf5"
else
    actual_file="${outdir}/mw_with_mu_eta_pt.hdf5"
fi
...
ln -sfn "${actual_file}" "${outdir}/alpha_s_w_histmaker.hdf5"
