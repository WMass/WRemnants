...
Weight construction is numpy (once at construction time); runtime contractions
are ``tf.tensordot`` / ``tf.einsum``.

1. :func:`dense_index_map` — ``(NQ, NY, NqT)`` int array mapping each
   rectangular grid cell to a flat sparse-btgrid bin index; ``-1`` marks
   missing combos. ``tf.gather`` with a sentinel pads a sparse ``(Nbins,)`` σ
   into a dense ``(NQ, NY, NqT)``.

2. :func:`q_integrate_weights` — ``(NQ,)`` weight vector for ``arctan_Q²``
   Simpson integration over the Z mass window. Apply via
   ``tf.einsum('q, qyz -> yz', w, sigma)``.

3. :func:`rebin_weights` — given a fine source grid and a coarser target-bin
   edge list, a ``(N_target, N_source)`` Simpson weight matrix. Apply via
   ``tf.tensordot``.
"""
...
# Z resonance parameters for the Q-integration change of variable,
# s-dependent-width scheme (see wremnants.utilities.common). Set only the centre
# and scale of the arctan-Q^2 transform; do not change the physics.
...
def dense_index_map(bins, Q_unique=None, Y_unique=None, qT_unique=None):
    ...
    ----------
    bins : list of tuples
        From ``load_btgrid_shards``: each ``(Q, Y, qT, lep)``, sorted
        lexicographically.
...
def sparse_to_dense_tf(sigma_flat, flat_idx):
    ...
    Missing cells (``flat_idx == -1``) padded with 0, via ``tf.gather`` with a
    0-padded sentinel row.
    """
    sigma_flat = _as_dtype(sigma_flat)
    # Append one "zero" entry that the -1 indices gather.
...
def q_integrate_weights(
    ...
    Same change of variable as the numpy reference
    ``scetlib_run.factorize.integrate_over_Q`` with ``method="arctan_Q2"``:
    x = arctan((Q² - q0²) / (q0 Γ)) flattens the Breit-Wigner peak, then Simpson
    on x with the Jacobian dQ/dx.

    Returns a ``(NQ,)`` weight vector, zero outside ``[Q_lo, Q_hi]``.
    """
...
def rebin_weights(source_grid, target_edges, name="axis", tol=1e-9):
    ...
    For each target bin ``[target_edges[i], target_edges[i+1]]``, find the source
    samples within ``tol`` slack of the bin's interior + edges and compute their
    Simpson weights. Dense matrix; entries are 0 for source samples not
    contributing to a target bin.

    Mirrors the per-bin call pattern of the numpy reference
...
def rebin_axis_tf(values, axis, weights):
    ...
    if axis < 0:
        axis += rank
    # tensordot contracts values[axis] with weights[1]; target axis lands at the
    # END of the result. Permute it back to position ``axis``.
