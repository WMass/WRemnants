...
def get_rebin_actions(axes, ax_lim=[], ax_rebin=[], ax_absval=[], rename=False):
    ...
    if len(ax_lim)>0 or len(ax_rebin)>0:
        actions.append(lambda h,axes=axes,rebins=ax_rebin,lows=ax_lim[::2],highs=ax_lim[1::2]: rebinHistMultiAx(h, axes, rebins, lows, highs))
...
def expand_hist_by_duplicate_axis(href, ref_ax_name, new_ax_name, swap_axes=False, put_trailing=False, flow=True):
    ...
    if put_trailing:
        ...
        new_axis_idx=0
    if swap_axes:
        new_axes[new_axis_idx], new_axes[ref_ax_idx+1] = new_axes[ref_ax_idx+1], new_axes[new_axis_idx]
...
def swap_axes(hist, axis1_name, axis2_name):
    
    axis1 = hist.axes.name.index(axis1_name)
    axis2 = hist.axes.name.index(axis2_name)
    
    # Check if axes are valid
    if not (0 <= axis1 < hist.ndim and 0 <= axis2 < hist.ndim):
        raise ValueError("Invalid axis indices")

    # Copy histogram to modify
    new_hist = hist.copy()

    # Swap axis metadata
    new_hist.axes[axis1], new_hist.axes[axis2] = new_hist.axes[axis2], new_hist.axes[axis1]

    # Swap data axes
    new_hist._storage = new_hist._storage.transpose(axis1, axis2)

    return new_hist

def move_and_insert_axis(hist1, from_index, to_index):
    """
    Move an axis from one position to another and insert it into the new position in a Boost-Histogram.

    Parameters:
    - hist (boost_histogram.Histogram): The Boost-Histogram to modify.
    - from_index (int): The index of the axis to move.
    - to_index (int): The index where the axis should be inserted.

    Returns:
    - boost_histogram.Histogram: The modified histogram with the axis moved and inserted.
    """
    # Check if indices are valid
    if not (0 <= from_index < hist1.ndim and 0 <= to_index <= hist1.ndim):
        raise ValueError("Invalid axis indices")

    # Copy histogram to modify
    new_hist = hist1.copy()
    
    # Remove axis from current position
    axis = new_hist.axes[from_index]

    # Remove the axis tuple and update the axes variable
    new_axes = list(new_hist.axes)
    new_axes.pop(from_index)
    new_axes.insert(to_index, axis)
    
    new_data = np.moveaxis(hist1.values(),from_index,to_index)
    new_hist = hist.Hist(*new_axes, data=new_data)
