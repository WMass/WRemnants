...
def dump_smoothing_params(
    ...
    (FakeSelector1DExtendedABCD, with flow=True, after y-axis flip so tight iso is last):
        0=Bx (low mt, pass iso), 1=Ax (low mt, fail iso),
        2=B  (mid mt, pass iso), 3=A  (mid mt, fail iso),
    ...
    Mapping to model order [A=0, B=1, C=2, Ax=3, Bx=4]:
        model_A  ← sideband flat 3
        model_B  ← sideband flat 2
        model_C  ← sideband flat 4
        model_Ax ← sideband flat 1
        model_Bx ← sideband flat 0
    """
    ...
    # Single call with signal_region=False: returns 5 regions [Bx=0, Ax=1, B=2, A=3, C=4]
    ...
    # Shape: (*outer_dims, 5, order+1) — indices [Bx=0, Ax=1, B=2, A=3, C=4]
    ...
    # A ← flat 3, B ← flat 2, C ← flat 4, Ax ← flat 1, Bx ← flat 0
    params_model = np.stack(
        [
            params_5d_flat[:, 2, :],  # B  (mid mt, pass iso)
            params_5d_flat[:, 3, :],  # A  (mid mt, fail iso)
            params_5d_flat[:, 4, :],  # C  (signal mt, fail iso) = application region
            params_5d_flat[:, 0, :],  # Bx (low mt, pass iso)
            params_5d_flat[:, 1, :],  # Ax (low mt, fail iso)
        ],
        axis=1,
    )  # (n_outer, 5, order+1)
