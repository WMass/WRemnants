...
parser.add_argument("--flavor", default="mumu", choices=["mumu", "ee"], help="Lepton flavor")
...
# load muonscarekit correction functions
...
def build_graph(df, dataset):
    ...
    # apply muonscarekit momentum corrections before selection
    if dataset.is_data:
        df = df.Define(
            "Muon_pt_corr",
    ...
    # event selection (using corrected pt)
    df = df.Filter("HLT_HIMu17")
    df = df.Define(
        "goodMu",
        "Muon_pt_corr > 18 && abs(Muon_eta) < 2.4 && Muon_mediumId && Muon_isGlobal"
    )
    # get events with exactly two muons that fit the criteria
    ...
    # want muons to have opposite charge
    ...
    # ---- Build dimuon kinematics (using corrected pt) ----
    MU_MASS = 0.105658
    df = (
        df.Define("mu0_p4", f"ROOT::Math::PtEtaPhiMVector(Muon_pt_corr[i0], Muon_eta[i0], Muon_phi[i0], {MU_MASS})")
          .Define("mu1_p4", f"ROOT::Math::PtEtaPhiMVector(Muon_pt_corr[i1], Muon_eta[i1], Muon_phi[i1], {MU_MASS})")
    ...
    # ---- Build CS angles (using corrected pt) ----
    df = (
        df.Define("mupos_p4", f"ROOT::Math::PtEtaPhiMVector(Muon_pt_corr[i_pos], Muon_eta[i_pos], Muon_phi[i_pos], {MU_MASS})")
          .Define("muneg_p4", f"ROOT::Math::PtEtaPhiMVector(Muon_pt_corr[i_neg], Muon_eta[i_neg], Muon_phi[i_neg], {MU_MASS})")
